...
Supporting files referenced by the workflow (references/) are installed with the skill at
`~/.claude/skills/test-challenger/` — read them from there when a step points to one.
