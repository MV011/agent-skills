...
Supporting files referenced by the workflow (config/, references/) are installed with the skill at
`~/.claude/skills/comprehensive-pr-review/` — read them from there when a step points to one.
