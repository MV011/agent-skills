...
## Tier → model mapping (GPT 5.6 & GPT-6 Astra lineup, live-verified September 2026)

| Tier | Model ID | Effort | Role | Key constraints |
|---|---|---|---|---|
| `cheap` | `gpt-5.6-luna` | `low` (`medium` for larger shards) | Triage, mechanical checks, patch application | Cheapest/fastest. Supports low…max; `ultra` accepted but may silently clamp |
| `standard` | `gpt-5.6-terra` | `medium`–`high` | Most review dimensions | High everyday capability at economical cost |
| `strong` | `gpt-5.6-sol` | `high`–`xhigh` | Adjudication-grade review, universal fallback | Flagship reliability. Also the vision tier — GPT vision outperforms Claude Opus 4.8 on screenshot/UI review. When quota allows, `gpt-6-astra` (`medium`/`high`) can also serve here |
| `deep` | `gpt-6-astra` | `max` (or `xhigh`) | Risk-surface (security/migration) review | Highest-tier flagship model for demanding tasks. Native reasoning levels: `low`, `medium`, `high`, `xhigh`, `max` (default: `medium`). Fallback: `gpt-5.6-sol` at `max` |
...
Do **not** route to `gpt-5.5` or older generations — 5.6 and 6.x Astra only. If a model misbehaves on a task, fall back within the family (astra ↔ sol ↔ terra), never to a prior generation.
...
3. **Escalation ladder (one rung at a time):** luna low → luna medium → terra medium → terra high → sol high → sol xhigh → astra high → astra max. Same promotion triggers as the Claude ladder (self-reported low confidence, or gate fails twice). `ultra` is not on the ladder — it is a deliberate coordinator decision, never an automatic escalation.
...
5. **Refusal/failure handling is unchanged** (see SKILL.md): GPT models don't share Claude's cyber-classifier refusal profile, but API errors, rate limits, and declines still happen — retry once on the task's fallback tier, tag `degraded: true`, record the producing model (e.g. `gpt-6-astra (max)` or `gpt-5.6-terra (high)`) in the findings table.
6. **Vision dimensions route to sol or astra.** If the PR includes UI screenshots, visual diffs, or asset changes needing visual judgment, attach images with `-i <file>` and use `strong` or `deep` regardless of the dimension's normal tier.
...
- Log to a file; poll `kill -0 $CODEX_PID` + `tail`; enforce a timeout (scale up for sol max or astra max/ultra).
